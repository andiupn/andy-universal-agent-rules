...
```bash
python .agent/scripts/save-knowledge.py --category [kategori] "[isi pengetahuan lengkap]"
```
...
- [ ] **Sync statistics AGENTS.md** (`python .agent/scripts/sync-agents-stats.py`)
...
```bash
# Gotcha
python .agent/scripts/save-knowledge.py --category gotchas "CRITICAL: [deskripsi masalah + solusi]"

# Pattern
python .agent/scripts/save-knowledge.py --category patterns "PATTERN: [nama pattern + cara pakai]"

# Decision
python .agent/scripts/save-knowledge.py --category decisions "DECISION: [keputusan + alasan]"
